...
/* KOSWeb: fit wide layouts to the screen. See the comment at the clamp in
 * calculate_containing_block_context for the reasoning.
 *
 * Runtime rather than compile-time so the two behaviours can be compared on
 * one page without rebuilding. That matters because this is a deliberate
 * departure from CSS made on judgement, and judgement should be checkable:
 * if a page reads worse clamped, it should take one button press to see that
 * rather than an argument. Defined here rather than in a header so the
 * vendored headers stay byte-identical to upstream. */
namespace litehtml
{
    bool ps_fit_to_viewport = true;
}
...
{
    // KOSWeb: typeid is unavailable under -fno-rtti. This is the debug dump
    // path only. If render-tree type names become useful for layout debugging,
    // restore them with a virtual dump_type_name() rather than RTTI.
    cout.begin_node(src_el()->dump_get_name());
    ...
    const containing_block_context& cb_context)
{
    ...
    //
    // A table cell is excluded: its width comes from the column algorithm,
    // which has already measured every cell in the column, so honouring one
    // cell's own width property here would fight that result.
    //
    // KOSWeb: height used to be calculated inside this same block, so it was
    // skipped for cells along with width - leaving ret.height auto, which is
    // then never applied in render_item_block::_render. A td with
    // height:64px laid out at its content height instead. Only width has the
    // column-algorithm conflict; a cell's height is its own business, and CSS
    // 2.1 17.5.3 treats it as a minimum for the row. So the height
    // calculation now sits outside the guard.
    ...
    // Calculate height if css property is not auto. Applies to every display
    // type, table cells included - see the note above.
    {
    ...
    // KOSWeb: nothing may be wider than the box that contains it.
    //
    // The min-width clamp below handles a page that declares a minimum. This
    // handles the far more common case: a fixed width, usually on a table,
    // wider than the screen - the forum at bb.dreampipe.net asks for 781
    // pixels, and the 141 past the right edge are unreachable on a console
    // with no horizontal scrolling.
    //
    // Applied to the width an element establishes for its own content, so it
    // cascades: an over-wide table is pulled in, and its columns then reflow
    // to the room actually available rather than to the room it asked for.
    if(ps_fit_to_viewport &&
       ret.width.type == containing_block_context::cbc_value_type_absolute &&
       ret.width.value > cb_context.width.value)
    {
        ret.width.value = cb_context.width.value;
    }
    ...
    // KOSWeb: a min-width wider than its containing block is clamped to it.
    //
    // This is a deliberate departure from CSS, and it is the difference
    // between a usable page and an unusable one on this hardware. The screen
    // is 640x480 and cannot scroll sideways, so when a page declares
    // something like min-width:800px the honest answer puts 160 pixels of it
    // permanently past the right edge, unreachable by any input the console
    // has. Worse, anything centred inside that box is centred against 800
    // rather than 640, so the visible part starts pushed right and clipped -
    // which reads as a broken browser rather than a page too wide to fit.
    //
    // Desktop browsers do the same arithmetic; the breakage is simply
    // invisible to them because their viewport is wider than the declared
    // minimum. Ours never is.
    //
    // Clamping reflows the page to the screen instead. Content that would
    // have been unreachable becomes readable, and the cost is only that a
    // layout demanding more room than exists does not get it - which was
    // never on offer here. Revisit if horizontal scrolling is ever added,
    // though on a d-pad that is a poor consolation.
    if(ps_fit_to_viewport &&
       ret.min_width.type != containing_block_context::cbc_value_type_none &&
       ret.min_width.value > cb_context.width.value)
    {
        ret.min_width.value = cb_context.width.value;
    }

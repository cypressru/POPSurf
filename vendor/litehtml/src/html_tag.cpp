...
namespace litehtml
{
    ...
    bool html_tag::get_custom_property(string_id name, css_token_vector& result) const
    {
        ...
        // KOSWeb: is_html_tag() replaces dynamic_cast under -fno-rtti.
        element* _p = parent().get();
        if(_p && _p->is_html_tag())
        {
            return static_cast<html_tag*>(_p)->get_custom_property(name, result);
        }
        return false;
    }
